...
public class DeliveryToShelfControllerQRCodeReader
{
...
public class DeliveryToShelfController : MonoBehaviour
{
    ...
    IEnumerator ApproachAndGrabBox()
    {
        if (targetBox == null)
        {
            Debug.LogError("Nessuna box selezionata per l'approccio!");
            yield break;
        }

        // Calcolo del punto di approccio rispetto al QR code
        Vector3 approachPoint = GetQRCodeApproachPoint(targetBox);
        Debug.Log($"Calcolato punto di approccio rispetto al QR code: {approachPoint}");

        // Vai al punto di approccio
        agent.ResetPath();
        agent.SetDestination(approachPoint);
        yield return new WaitUntil(() => !agent.pathPending && agent.remainingDistance <= agent.stoppingDistance);

        if (Vector3.Distance(transform.position, approachPoint) > 0.2f)
        {
            Debug.LogError("Errore: non sono riuscito a raggiungere il punto di approccio!");
            yield break;
        }

        Debug.Log($"Arrivato al punto di approccio della box {targetBox.name}. Rotazione in corso...");

        // Rotazione verso il lato con il QR code
        Vector3 targetDirection = targetBox.transform.right; // Direzione opposta alla faccia del QR code
        yield return StartCoroutine(SmoothRotateToDirection(targetDirection, 1f));
        Debug.Log("Rotazione completata. Allineamento con il QR code garantito.");

        // Pausa per la lettura del QR code
        Debug.Log("In attesa di 5 secondi per lettura del QR code...");
    ...
    Vector3 GetQRCodeApproachPoint(GameObject box)
    {
        // Supponiamo che il QR code sia sulla faccia destra della box
        Vector3 boxPosition = box.transform.position;
        Vector3 qrCodeDirection = box.transform.right; // Direzione verso destra (dove si trova il QR code)
        float distance = 3f; // Distanza dall'approccio
        return boxPosition - qrCodeDirection * distance;
    }
    ...
    private void OnDrawGizmos()
    {
        if (targetBox != null)
        {
            ...
            Gizmos.DrawSphere(approachPoint, 0.1f);
